...
protected:
  void initGl() override
  {
	...
	// client
	client = std::unique_ptr<ClientGame>(new ClientGame());
  ...
  void renderScene(const glm::mat4& projection, const glm::mat4& headPose) override
  {
	 // Model Positions
	  ...
	  if (client->other_done && client->my_done && gameMode == PREPARE) {
	  ...
	  if (client->other_attack.first != -1) {
		  if (scene->myBoard[client->other_attack.first][client->other_attack.second] == scene->MARKED) {
			  scene->myBoard[client->other_attack.first][client->other_attack.second] = scene->SHOOTED;
			  client->my_damage.first = client->other_attack.first;
			  client->my_damage.second = client->other_attack.second;
			  client->other_attack.first = -1;
			  client->other_attack.second = -1;
			  scene->numDamages++;
		  }
		  else {
			  scene->myBoard[client->other_attack.first][client->other_attack.second] = scene->MISSED;
		  }
	  }
	  
	  
	  if (client->other_damage.first != -1) {
		  scene->rivalBoard[client->other_damage.first][client->other_damage.second] = scene->SHOOTED;
		  client->other_damage.first = -1;
		  client->other_damage.first = -1;
		  scene->numHits++;
	  }
	  ...
	  client->my_headPose = HeadPose;
	  otherHead->toWorld = client->other_headPose * glm::scale(glm::mat4(1.0f), glm::vec3(0.02f));
  ...
  void UpdateButtonY() {
	  if (scene->rivalBoard[scene->x_cord][scene->y_cord] == 0 && playerMode == MY) {
		  ...
		  client->my_attack.first = scene->x_cord;
		  client->my_attack.second = scene->y_cord;
